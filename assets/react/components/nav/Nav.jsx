  ...
  return (
    <nav className="navbar navbar-expand-md bg-transparent position-absolute top-0 w-100" id="client-nav">
      <div className="container">
        <Link to="/" className="navbar-brand  text-white hero-fs-25">
          {process.env.NAV_TITLE}
        </Link>
        ...
        <div className="collapse navbar-collapse hero-fs-20 z-3 p-2" id="navbarSupportedContent" ref={collapseRef}>
          <ul className="navbar-nav me-auto mb-2 mb-lg-0" onClick={() => collapseRef.current.classList.remove('show')}>
            ...
            {user !== null && (
              <li className="nav-item">
                <NavLink to="/categories" className={({ isActive }) => getRouteClassNames(isActive)}>
                  Catégories
                </NavLink>
              </li>
            )}
            {user?.roles.includes('ROLE_ADMIN') && (
              <li className="nav-item">
                <NavLink to="/admin_v2/users" className={({ isActive }) => getRouteClassNames(isActive)}>
                  Admin
                </NavLink>
              </li>
            )}
          ...
          <div className="d-flex align-items-center">
            <i
              className="bi bi-arrow-clockwise hero-cursor-pointer text-white me-3"
            ...
            {deferredPrompt !== null && (
              <i
                className="bi bi-download hero-cursor-pointer text-white me-3"
                data-bs-toggle="tooltip"
                data-bs-placement="bottom"
                data-bs-title="Installer"
            ...
            {user !== null && (
              <button type="button" className="btn btn-outline-dark btn-sm ms-3" onClick={() => logout()}>
                Déconnexion
              </button>
            )}
          </div>
