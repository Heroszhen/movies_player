                  ...
                  return (
                    <div className="col-md-6 col-lg-3 mb-4" key={index}>
                      <div className="card border border-0">
                        {item.currentPhoto && (
                          <img
                            src={`${process.env.AWS_FILE_PREFIX}${item.currentPhoto.imageName}`}
                            alt=""
                            className="card-img-top"
                          />
                        )}
                        {!item.currentPhoto && (
                          <img src="/build/static/poster_not_found.png" alt="" className="card-img-top" />
                        )}
                        <div className="card-body hero-bg-color-edf5f7 p-4">
                          <NavLink
                            to={'/acteur/' + item.id}
                            className="hero-color-212529 text-decoration-none fs-3 hover:hero-color-9352b3"
                            onClick={(e) => viewOneActor(e, item.id)}>
                            {item.name}
                          </NavLink>
                        </div>
                      </div>
